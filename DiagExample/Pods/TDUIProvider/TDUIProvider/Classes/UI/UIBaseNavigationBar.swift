...
@objc open class UIBaseView: UIView {
...
@objc(UIBaseNavigationBar)
public class UIBaseNavigationBar: UIBaseView {
    ...
    // 左边返回标题
    @objc public var leftTitleStr: String? {
        didSet {
            leftTitleLabel.text = leftTitleStr
            leftTitleLabel.sizeToFit()
        }
    }
    // 右边按钮标题
    @objc public var rightStr: String? {
        didSet {
            rightBtn.isHidden = false
            rightBtn.setTitle(rightStr, for: .normal)
        }
    }
    ...
    var rightBtn: UIScrollButton!
    var bottomLine: UIView!
    var callback: Callback = Callback()
    
    @objc public override func setupUI() {
        super.setupUI()
        theme.backgroundColor = .theme.navigationBackground
        ...
        self.backBtn = {
            ...
            btn.theme.setImage(UIProvider.navigationBackIcon, forState: .normal)
        ...
        self.leftTitleLabel = {
            let label = UILabel(text: nil, themeColor: .theme.subTitle, font: .systemFont(ofSize: 16, weight: .medium).adaptHD)
            addSubview(label)
        ...
        self.titleLabel = {
            var label = UILabel(text: nil, themeColor: .theme.title, font: .systemFont(ofSize: 16, weight: .medium).adaptHD)
        ...
        self.rightBtn = {
            let btn = UIScrollButton(type: .custom)
            btn.theme.setTitleColor(.theme.navigationRightItemsText, forState: .normal)
            btn.titleLabel?.font = .systemFont(ofSize: 16).adaptHD
...
@objc(UISrollDirection)
public enum UISrollDirection: Int {
    case left, right, comeAndBack
}

@objc(UIScrollButton)
public class UIScrollButton: UIButton {
    /// 内边距
    @objc public var margin: CGFloat = 10
    /// 滚动方向 默认 left
    @objc public var scrollDirection: UISrollDirection = .left {
        didSet {
            removeLink()
            setNeedsDisplay()
            setNeedsLayout()
    ...
    /// 设置badge的字体
    @objc open var badgeFont: UIFont = .systemFont(ofSize: 9).adaptHD{
        didSet {
            updateBadge()
        }
    }
