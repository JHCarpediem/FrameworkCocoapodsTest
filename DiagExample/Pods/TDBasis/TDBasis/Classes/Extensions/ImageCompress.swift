...
public extension ImageCompress {
    ...
    static func compressImageData(_ rawData: Data, sampleCount: Int) throws -> Data {
        ...
        // 合并帧的时间,最长不可高于 200ms
        let filter = (0 ..< frameDurations.count).filter { $0 % sampleCount == 0 }
        let mergeFrameDurations = filter.map { min(frameDurations[$0 ..< min($0 + sampleCount, frameDurations.count)].reduce(0.0) { $0 + $1 }, 0.2) }
