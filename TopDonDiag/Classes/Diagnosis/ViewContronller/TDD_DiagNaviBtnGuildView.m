...
@interface TDD_DiagNaviBtnGuildView ()
...
@implementation TDD_DiagNaviBtnGuildView
...
- (instancetype)initWithDiagType:(TDD_DiagNavType)diagNavType {
    ...
    if (self) {
        ...
        self.highlightImageArray = @[@"navi_more_diagnosis", @"nav_search_ic", @"navi_translate_normal", @"navi_feedback_diagnosis"];
        self.tipArray = @[TDDLocalized.diagnose_guide_tip_one, TDDLocalized.diagnose_guide_tip_two, TDDLocalized.diagnose_guide_tip_four, TDDLocalized.diagnose_guide_tip_three];
...
- (void)setupUI {
    ...
    if (self.diagNavType == TDD_DiagNavType_IMMO) {
        self.popImageView.image = kImageNamed(@"guide_btn_immo_pop");
    }
    
    [self.highlightView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.equalTo(self).offset(-30);
        make.width.mas_equalTo(self.diagNavType == TDD_DiagNavType_IMMO ? 188 : 132);
...
- (void)nextBtnClick {
    if (self.guildStep < self.tipArray.count - 1) {
        ...
        if (self.guildStep == 1) {
            ...
            if (self.block && self.diagNavType != TDD_DiagNavType_IMMO) {
                self.block(NO);
            }
        }
        ...
        // liuyong:（103 和 118.4） 分别是平板和手机的 NaviMoreBtnView 里面的，48 是按钮的宽+间距，(18和 15) 是按钮的一半宽度，12 是 rightBtnView 按钮的间距
        CGFloat right = IS_IPad ? -118.4 - 10 - 18 : -103 -10 - 18;
        CGFloat top = NavigationHeight + 5;
        if (self.guildStep == 2) {
            right = IS_IPad ? -118.4 + 48 - 10 - 15 : -60 - 18;
        } else if (self.guildStep == 3) {
            right = IS_IPad ? -36 * 1.18 - 5 - 12 - 15 : -50 - 18;
            top = StatusBarHeight + 4;
        }
        if (self.diagNavType == TDD_DiagNavType_IMMO) {
            top = NavigationHeight + 5;
            right = IS_IPad ? -118.4 + (self.guildStep - 2) * 48 : -103 + (self.guildStep - 2) * 48;
        }
...
- (UIImageView *)popImageView {
    if (!_popImageView) {
        ...
        _popImageView.image = kImageNamed(@"guide_btn_pop");
    }
