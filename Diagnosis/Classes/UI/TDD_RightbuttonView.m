...
- (instancetype)initWithType:(TDD_DiagNavType)diagNavType {
    ...
    if (self) {
        ...
        _vciH = IS_IPad ? 30 : ([TDD_DiagnosisTools softWareIsCarDiag] ? 44 : 36);
...
- (void)setupUI {
    ...
    [self.vciStatusBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        if ([TDD_DiagnosisTools softWareIsCarDiag]) {
    ...
    if (showBtnArray.count == 1) {
        ...
        if (type.intValue == kNavSaveBtn) {
            ...
            [self.vciStatusBtn mas_remakeConstraints:^(MASConstraintMaker *make) {
                make.centerY.equalTo(self);
                make.right.equalTo(self).offset(-5);
                make.width.mas_equalTo(self.diagNavType == TDD_DiagNavType_TDarts ? 72 : ([TDD_DiagnosisTools softWareIsCarDiag] ? 44 : _vciW));
                if ([TDD_DiagnosisTools softWareIsCarDiag]) {
                    make.height.mas_lessThanOrEqualTo(NavigationHeight);
                }else {
                    make.height.mas_equalTo(_vciH);
...
//    }
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (TDD_DiagnosisTools.softWareIsKindOfDiagScan && TDD_EADSessionController.sharedController.VciStatus) {
