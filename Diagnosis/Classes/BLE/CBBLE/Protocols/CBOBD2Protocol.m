...
@implementation CBOBD2ProtocolResult
{
...
{
    CBOBD2ProtocolResult* obj = [[self alloc] init];
...
@implementation CBOBD2Protocol

+(instancetype)protocol
{
    CBOBD2Protocol* obj = [[self alloc] init];
    return obj;
}
...
-(NSDictionary<NSString*,CBOBD2ProtocolResult*>*)decode:(NSArray<NSString*>*)lines originatingCommand:(NSString*)command
{
    NSAssert( NO, @"please implement decode:originatingCommand: in your subclass" );
    return nil;
}

-(CBOBD2Command*)heartbeatCommand
{
    return nil;
}
...
-(CBOBD2ProtocolResult*)createProtocolResultForBytes:(NSArray<NSNumber*>*)bytes sidIndex:(NSUInteger)sidIndex
{
    uint sid = bytes[sidIndex].unsignedIntValue;
    if ( sid != OBD2FailureCode )
    {
        return [CBOBD2ProtocolResult protocolResultFailureType:OBD2FailureTypeInternalOK];
    }
    if ( sidIndex + 2 >= bytes.count )
    {
        return [CBOBD2ProtocolResult protocolResultFailureType:OBD2FailureTypeInternalUnknown];
    }
    __unused uint failedPid = bytes[sidIndex + 1].unsignedIntValue;
    uint failureType = bytes[sidIndex + 2].unsignedIntValue;
    
    return [CBOBD2ProtocolResult protocolResultFailureType:failureType];
}
