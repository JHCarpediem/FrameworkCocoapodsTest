...
@interface CBOBD2AdapterInternalCommand : NSObject
{
    NSDate* _startDate;
}

+(instancetype)commandWithOBD2Command:(CBOBD2Command*)command responseHandler:(CBOBD2CommandResponseHandler)responseHandler;

@property(strong,nonatomic,readonly) CBOBD2Command* command;
@property(strong,nonatomic,readonly) CBOBD2CommandResponseHandler responseHandler;

@end

@implementation CBOBD2AdapterInternalCommand

+(instancetype)commandWithOBD2Command:(CBOBD2Command *)command responseHandler:(CBOBD2CommandResponseHandler)responseHandler
{
    CBOBD2AdapterInternalCommand* obj = [[self alloc] init];
...
-(void)didCompleteResponse:(NSArray<NSString*>*)lines protocol:(CBOBD2Protocol*)protocol protocolType:(OBD2VehicleProtocol)protocolType
{
...
#pragma mark -
#pragma mark CBOBD2Adapter

NSString* const CBOBD2AdapterDidUpdateState = @"CBOBD2AdapterDidUpdateState";
NSString* const CBOBD2AdapterDidOpenStream = @"CBOBD2AdapterDidOpenStream";
NSString* const CBOBD2AdapterDidSend = @"CBOBD2AdapterDidSend";
NSString* const CBOBD2AdapterDidReceive = @"CBOBD2AdapterDidReceive";

@implementation CBOBD2Adapter
{
    ...
    CBOBD2Protocol* _adapterProtocol;
...
{
    CBOBD2Command* command = [CBOBD2Command commandWithRawString:rawString];
    [self transmitCommand:command responseHandler:^(CBOBD2Command * _Nonnull command) {
...
-(void)transmitCommand:(CBOBD2Command*)command responseHandler:(nullable CBOBD2CommandResponseHandler)handler
{
    CBOBD2AdapterInternalCommand* internalCommand = [CBOBD2AdapterInternalCommand commandWithOBD2Command:command responseHandler:handler];
    dispatch_async( _dispatchQueue, ^{
        [self asyncEnqueueInternalCommand:internalCommand];
    });
}

-(void)transmitMultipleCommands:(NSArray<CBOBD2Command*>*)commands responseHandler:(nullable CBOBD2CommandResponseHandler)handler
{
    if ( !commands.count )
    {
        return;
    ...
    [commands enumerateObjectsUsingBlock:^(CBOBD2Command * _Nonnull command, NSUInteger idx, BOOL * _Nonnull stop) {
        [self transmitCommand:command responseHandler:handler];
    }];
}

-(void)transmitMultipleCommands:(NSArray<CBOBD2Command*>*)commands completionHandler:(nullable LTOBD2MultipleCommandsResponseHandler)handler
{
    if ( !commands.count )
    {
        return;
    ...
    [commands enumerateObjectsUsingBlock:^(CBOBD2Command * _Nonnull command, NSUInteger idx, BOOL * _Nonnull stop) {
        CBOBD2CommandResponseHandler commandHandler = ( idx < commands.count - 1 ) ? nil : ^(CBOBD2Command* command ){
            handler( commands );
        };
...
-(void)advanceAdapterStateTo:(OBD2AdapterState)nextState
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:CBOBD2AdapterDidUpdateState object:self];
...
-(BOOL)sendCommand:(CBOBD2Command*)command
{
...
{
    [[NSNotificationCenter defaultCenter] postNotificationName:CBOBD2AdapterDidReceive object:self];
    ...
    CBOBD2AdapterInternalCommand* internalCommand = _commandQueue.firstObject;
...
-(void)didRecognizeProtocol:(OBD2VehicleProtocol)protocol
{
    ...
    switch ( protocol )
    {
        ...
        case OBD2VehicleProtocolCAN_11B_250K: /* P8 */
            implementation = [CBOBD2ProtocolISO15765_4 protocolVariantWith11BitHeaders];
            break;

        /* CAN w/ 29 bit headers, no checksum */
        case OBD2VehicleProtocolCAN_29B_500K: /* P7 */
        case OBD2VehicleProtocolCAN_29B_250K: /* P9 */
            implementation = [CBOBD2ProtocolISO15765_4 protocolVariantWith29BitHeaders];
            break;
        ...
        case OBD2VehicleProtocolKWP2000_5KBPS: /* P4 */
            implementation = [CBOBD2ProtocolISO14230_4 protocol];
            break;
            
        /* SAE J1850 (V)PWM */
        case OBD2VehicleProtocolJ_1850PWM:  /* P1 */
        case OBD2VehicleProtocolJ_1850VPWM: /* P2 */
            implementation = [CBOBD2ProtocolSAEJ1850 protocol];
            break;
            
        /* ISO 9141-2 K-LINE*/
        case OBD2VehicleProtocolISO_9141_2: /* P3 */
            implementation = [CBOBD2ProtocolISO9141_2 protocol];
            break;
...
-(void)stream:(NSStream*)stream handleEvent:(NSStreamEvent)eventCode
{
    ...
    {
        [[NSNotificationCenter defaultCenter] postNotificationName:CBOBD2AdapterDidOpenStream object:stream];
    }
...
-(void)asyncProcessCommandQueue
{
    ...
    if ( [self sendCommand:internalCommand.command] )
    {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:CBOBD2AdapterDidSend object:self];
    }
}

-(void)asyncEnqueueInternalCommand:(CBOBD2AdapterInternalCommand*)internalCommand
{
