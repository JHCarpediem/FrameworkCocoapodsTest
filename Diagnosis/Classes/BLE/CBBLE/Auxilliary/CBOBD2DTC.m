...
static NSDictionary<NSString*,NSString*>* CBOBD2DTCEcuDictionary;

@implementation CBOBD2DTC
...
+(void)initialize
{
    if ( self != CBOBD2DTC.class )
    {
        return;
    }
    
    CBOBD2DTCEcuDictionary = @{
                                         @"10": @"ECU #1",
                                         @"17": @"ECU #2",
                                         @"1E": @"ECU #3",
                                         @"7E8": @"ECU #1",
...
{
    return [CBOBD2DTCEcuDictionary objectForKey:_ecu] ?: [NSString stringWithFormat:@"ECU @ 0x%@", _ecu];
}
