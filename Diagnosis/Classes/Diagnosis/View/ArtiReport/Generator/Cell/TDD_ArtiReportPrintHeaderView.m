...
//    iconImageView.center = CGPointMake(iconImageView.center.x, self.center.y);
    ...
    [self addSubview:self.diagIdLabel];
    ...
    [self.diagIdLabel mas_makeConstraints:^(MASConstraintMaker *make) {
    ...
    if ([TDD_DiagnosisManage sharedManage].currentSoftware == TDDSoftwareDeepDiag) {
    ...
    NSString *diagId = [TDUserModel current].diagId;
    if ([NSString tdd_isEmpty:[TDUserModel current].diagId]) {
        diagId = @"--";
    }
    if ([TDD_DiagnosisManage sharedManage].currentSoftware == TDDSoftwareDeepDiag) {
        self.diagIdLabel.text = [NSString stringWithFormat:@"ID: %@", diagId];
    } else {
        self.diagIdLabel.text = [NSString stringWithFormat:@"diag ID: %@", diagId];
    }
}
...
- (TDD_CustomLabel *)diagIdLabel {
    if (!_diagIdLabel) {
        _diagIdLabel = [[TDD_CustomLabel alloc] init];
        _diagIdLabel.font = [UIFont systemFontOfSize:10];
        _diagIdLabel.textColor = [UIColor whiteColor];
    }
    return _diagIdLabel;
}
