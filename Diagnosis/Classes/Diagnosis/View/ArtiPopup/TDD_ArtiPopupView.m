...
- (instancetype)init {
    ...
    if (self) {
        ...
        if ([[TDD_DiagnosisManage sharedManage].carModel.strVehicle isEqualToString:@"EOBD"] && [TDD_ArtiGlobalModel sharedArtiGlobalModel].obdEntryType == OET_CARPAL_OBD_ENGINE_CHECK && [TDD_DiagnosisTools softWareIsCarDiagSeries]) {
...
//    cell.fd_enforceFrameLayout = NO;
    ...
    //AI 引导
    if ((([TDD_DiagnosisTools customizedType] == TDD_Customized_Germany) || ([TDD_DiagnosisManage sharedManage].functionConfigMask & 1) || ([TDD_DiagnosisTools softWareIsCarDiagSeries])) && indexPath.row == 0 && [cellView isKindOfClass:[TDD_ArtiPopupSuggestCellView class]]) {
