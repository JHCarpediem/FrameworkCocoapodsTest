...
{
    BOOL isLock = [TDD_DiagnosisTools isLimitedTrialFuction] && ![TDD_DiagnosisTools softWareIsCarDiagSeries];
...
//    self.stateView.stateStr = [TDD_DiagnosisManage getDtcNodeStatusDescription:(int)_itemModel.uStatus statusStr:self.isShowTranslated ?_itemModel.strTroubleStatus :_itemModel.strTranslatedTroubleStatus];
    ...
    if (![[UIViewController tdd_topViewController] isKindOfClass:[TDD_DiagnosisViewController class]] && [TDD_DiagnosisTools softWareIsCarDiagSeries]) {
...
- (void)setItemModel:(TDD_ArtiTroubleItemModel *)itemModel
{
    ...
    if ([TDD_DiagnosisTools isLimitedTrialFuction] && ![TDD_DiagnosisTools softWareIsCarDiagSeries]) {
...
- (void)buttonClick:(UIButton *)button
{
    ...
    //软件过期前往购买
    if ([TDD_DiagnosisTools isLimitedTrialFuction] && ![TDD_DiagnosisTools softWareIsCarDiagSeries] && tag > 1) {
        if ([self.delegate respondsToSelector:@selector(ArtiTroubleCellLockClick:)]) {
            [self.delegate ArtiTroubleCellLockClick:self];
            return;
        }
    ...
    if (tag == 2) {
        ...
        if (tag == 3){
            NSString *referrer = @"HealthCheck";
            if (![[UIViewController tdd_topViewController] isKindOfClass:[TDD_DiagnosisViewController class]] && [TDD_DiagnosisTools softWareIsCarDiagGuru]) {
                referrer = @"EngineInspection";
            }
