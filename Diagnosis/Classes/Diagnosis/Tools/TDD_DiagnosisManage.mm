...
+ (void)DiagnosisInit
{
    ...
    if ([TDD_DiagnosisManage sharedManage].appScenarios==AS_INTERNAL_USE) {
        [TDD_StdShowModel EnableLogcat:false];

        [TDD_StdCommModel SetLogEnable:false];
    }
...
+ (uint32_t)CarArtiWithCarModel:(TDD_CarModel *)carModel
{
    HLog(@"进入诊断");
    ...
    backInt = [TDD_DiagnosisManage CarArtiWithType:type Vehicle:carModel];
    ...
    HLog(@"CarArti 返回参数为：%d -- %@", backInt, msg);
...
+ (uint32_t)CarArtiWithType:(NSString *)Type Vehicle:(TDD_CarModel *)carModel
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:KTDDNotificationCarArtiStart object:dic];
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:KTDDNotificationCarArtiStop object:dic];
    
    [self sharedManage].carModel = nil;
    
    //JH: 跳车逻辑暂时仅类DiagScan项目，当isCurVehNotSupport为 YES 时。不清除车型 VIN 码，VIN 码用于跳车
    BOOL keepVIN = [TDD_DiagnosisTools softWareIsKindOfDiagScan] ? ([TDD_ArtiGlobalModel sharedArtiGlobalModel].isCurVehNotSupport) : NO;
...
+ (void)switchDBType:(TDD_DBType)dbType {
    if ([TDD_EADSessionController sharedController].isCarArti) {
        HLog(@"已经进车不允许切换数据库");
    }else {
...
+ (BOOL)StopCranking {
    ...
    if (isKindOfTopVCI) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:KTDDNotificationCarArtiStopCranking object:nil];
    }
...
+ (uint32_t )runMaigc:(NSString *)diagId magic:(NSString *)magic
{
    //JH:现在版本的 STDCOMM runMagic 会崩溃，先使用 setLock
    return [TDD_StdCommModel setLock];
//    return [TDD_StdCommModel runMaigc:diagId magic:magic];
}
