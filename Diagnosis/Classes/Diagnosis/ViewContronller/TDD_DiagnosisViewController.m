...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(artiDiagExit) name:KTDDNotificationCarArtiStop object:nil];
...
- (void)enterDiagnosis
{
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        uint32_t backInt = [TDD_DiagnosisManage CarArtiWithCarModel:self.carModel];
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if ([[self.carModel.strVehicle uppercaseString] isEqualToString:@"AUTOVIN"] || ([TDD_ArtiGlobalModel sharedArtiGlobalModel].isCurVehNotSupport && [TDD_DiagnosisTools softWareIsKindOfDiagScan])){
...
- (void)artiShow:(NSNotification *)info{
    ...
    if (([model isKindOfClass:[TDD_ArtiTroubleModel class]] || [model isKindOfClass:[TDD_ArtiPopupModel class]]) && [TDD_DiagnosisTools isLimitedTrialFuction] && [TDD_DiagnosisTools softWareIsKindOfDiagScan]) {
...
#warning:TODO 数据流数据由model保存
    ...
    if (![self.model isKindOfClass:[model class]] && !self.model.isShowOtherView && ![model isKindOfClass:[TDD_ArtiMiniMsgBoxModel class]] || ([self.model isKindOfClass:[TDD_FCAAuthModel class]])) {
        HLog(@"移除旧UI");
        if ([TDD_DiagnosisTools softWareIsCarDiagSeries] && [model isKindOfClass:[TDD_ArtiSystemModel class]]) {
            //Carpal 系统扫描曝光率埋点
...
//        diag_fca_login
    ...
    if ([model isKindOfClass:[TDD_ArtiMenuModel class]]) {
        ...
        //DiagScan 的 popUp 与 troubleModel 样式一致
        if ([TDD_DiagnosisTools softWareIsKindOfDiagScan]) {
...
- (void)vciStatusDidChange {
    ...
    if (([_model isKindOfClass:[TDD_ArtiTroubleModel class]] || [_model isKindOfClass:[TDD_ArtiPopupModel class]]) && [TDD_DiagnosisTools isLimitedTrialFuction] && [TDD_DiagnosisTools softWareIsKindOfDiagScan]) {
...
//        }
        ...
        if (([TDD_EADSessionController sharedController].VciStatus && ![[self.carModel.strVehicle uppercaseString] isEqualToString:@"AUTOVIN"] && self.diagNavType != TDD_DiagNavType_TDarts)) {
            //DiagScan 跳车不提示
            if ([TDD_ArtiGlobalModel sharedArtiGlobalModel].isCurVehNotSupport && [TDD_DiagnosisTools softWareIsKindOfDiagScan]) {
                return;
            }
            if (!isKindOfTopVCI) {
...
- (void)artiDiagStopSound {
    ...
    NSURL *voice = [bundle URLForResource:[NSString stringWithFormat:@"Diagnosis.bundle/%@",kSoundArtiStop] withExtension:nil];
...
- (void)enterForeGround:(NSNotification *)notif
{
    ...
    if ([TDD_ArtiGlobalModel sharedArtiGlobalModel].diagShowType == TDD_DiagShowType_TDarts) {
        if (![TDD_TDartsManage sharedManage].TProgStatus 
            && [TDD_EADSessionController sharedController].isCarArti
