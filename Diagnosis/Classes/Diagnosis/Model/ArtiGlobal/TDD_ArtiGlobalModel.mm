...
+ (void)registerMethod
{
    ...
    //小车探 以及 DiagScan4.56 及以后版本才注册这个接口
    if (isTopVCI || ([TDD_DiagnosisTools softWareIsKindOfDiagScan] && [[TDD_DiagnosisManage getVersion] compare:@"2.06.000" options:NSNumericSearch] == NSOrderedDescending)) {
...
//        model.dbType = [TDD_JKDBHelper shareInstance].dbType;
        model.diagId = [TDD_DiagnosisTools diagID];
...
//    #endif
    ...
    NSString * diagId = [TDD_DiagnosisTools diagID];
...
功能：诊断程序设置不支持当前VIN码车辆
      ...
      小车探/CarDiag，需立即进车OBD，DiagScan等需要根据SetVehicle的车型进入到指定
...
+ (void)ArtiGlobalModelSetEventTracking:(eEventTrackingId )trackingID param:(std::vector<stTrackingItem>)param {
    ...
    if (vehicleInfoModel && !vehicleInfoModel.didSave) {
        if ([[TDD_DiagnosisManage sharedManage].manageDelegate respondsToSelector:@selector(insertMessageToDiagLog:time:type:)]) {
            [[TDD_DiagnosisManage sharedManage].manageDelegate insertMessageToDiagLog:vehicleInfoModel.vehInfo time:vehicleInfoModel.setTime type:0];
    ...
    if (setVinModel && !setVinModel.didSave) {
        if ([[TDD_DiagnosisManage sharedManage].manageDelegate respondsToSelector:@selector(insertMessageToDiagLog:time:type:)]) {
            [[TDD_DiagnosisManage sharedManage].manageDelegate insertMessageToDiagLog:setVinModel.vinStr time:setVinModel.setTime type:1];
...
{
    //DiagScanV5.40及之后 所有 app 掩码为空时不做兜底逻辑
//    if (!_diagMenuMask) {
