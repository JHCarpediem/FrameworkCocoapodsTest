...
@implementation TDD_JKDBModel

#pragma mark - override method
+ (void)initialize
{
    if (self != [TDD_JKDBModel self]) {
        [self createTableWithDBType:TDD_DATA_BASE_TYPE_DEFAULT];
        [self createTableWithDBType:TDD_DATA_BASE_TYPE_GROUP];
...
+ (NSDictionary *)getPropertys
{
    ...
    objc_property_t *properties = class_copyPropertyList([self class], &outCount);//不包括父类
...
+ (NSDictionary *)getAllProperties
{
    ...
    [proNames addObject:appTypeKey];
    [proTypes addObject:[NSString stringWithFormat:@"%@ %@",SQLINTEGER,PrimaryKey]];
    [proTypes addObject:[NSString stringWithFormat:@"%@",SQLINTEGER]];
...
{
    __block BOOL res = NO;
    TDD_JKDBHelper *jkDB = [TDD_JKDBHelper shareInstance];
    [jkDB.dbQueue inDatabase:^(FMDatabase *db) {
        NSString *tableName = [TDD_JKDBModel saveClassName:NSStringFromClass(self.class)];
 ...
 * 如果已经创建，返回YES
 */
+ (BOOL)createTableWithDBType:(TDD_DBType)dbType {
    __block BOOL res = YES;
    TDD_JKDBHelper *jkDB = [TDD_JKDBHelper shareInstance];
    FMDatabaseQueue *queue = (dbType == TDD_DATA_BASE_TYPE_GROUP)?[jkDB groupQueueWithGroupID:jkDB.groupID]: [[FMDatabaseQueue alloc] initWithPath:[TDD_JKDBHelper dbPathWithDirectoryName:nil groupID:nil]];
    
    return [self createTableWithDBQueue:queue];
}
 ...
 * 如果已经创建，返回YES
 */
+ (BOOL)createTableWithDBQueue:(FMDatabaseQueue *)queue {
    __block BOOL res = YES;
...
//        NSPredicate *filterPredicate = [NSPredicate predicateWithFormat:@"NOT (SELF IN %@)",columns];
        NSPredicate *filterPredicate = [NSPredicate predicateWithFormat:@"NOT (lowercase(SELF) IN %@)",columns];
        //过滤数组
        ...
        for (NSString *column in resultArray) {
            ...
            NSString *sql = [NSString stringWithFormat:@"ALTER TABLE %@ ADD COLUMN %@ ",[TDD_JKDBModel saveClassName:NSStringFromClass(self.class)],fieldSql];
            if (![db executeUpdate:sql]) {
                res = NO;
                *rollback = YES;
                return ;
...
{
    TDD_JKDBHelper *jkDB = [TDD_JKDBHelper shareInstance];
    return [self createTableWithDBQueue:jkDB.dbQueue];
}
...
//+ (BOOL)createTable
//{
//    FMDatabase *db = [FMDatabase databaseWithPath:[TDD_JKDBHelper dbPath]];
//    if (![db open]) {
//        NSLog(@"数据库打开失败!");
//        return NO;
//    }
...
{
    //判断是否是JKBaseModel的子类
    for (TDD_JKDBModel *model in array) {
        if (![model isKindOfClass:[TDD_JKDBModel class]]) {
            return NO;
        }
    }
    ...
    [jkDB.dbQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        for (TDD_JKDBModel *model in array) {
            [model saveAppType];
            NSString *tableName = [TDD_JKDBModel saveClassName:NSStringFromClass(model.class)];
            NSMutableString *keyString = [NSMutableString string];
            NSMutableString *valueString = [NSMutableString string];
            NSMutableArray *insertValues = [NSMutableArray  array];
...
- (void)saveAppType {
    if ([TDD_DiagnosisTools softWareIsSingleApp] && _appType == TDD_DATA_BASE_GROUP_DEFAULT) {
        switch (TDD_DiagnosisManage.sharedManage.currentSoftware) {
            case TDDSoftwareDiagScanVAG:
                {
                    _appType = TDD_DATA_BASE_GROUP_VAG;
                }
                break;
            case TDDSoftwareDiagScanBMW:
                {
                    _appType = TDD_DATA_BASE_GROUP_BMW;
                }
                break;
            case TDDSoftwareDiagScanFORD:
                {
                    _appType = TDD_DATA_BASE_GROUP_FORD;
                }
                break;
            default:
                break;
...
+ (NSArray *)findAll
{
    ...
    [jkDB.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        while ([resultSet next]) {
            TDD_JKDBModel *model = [[self.class alloc] init];
            for (int i=0; i< model.columeNames.count; i++) {
...
#pragma mark - 去掉TDD_前缀
+ (NSString *)saveClassName:(NSString *)className {
    
    return [className stringByReplacingOccurrencesOfString:@"TDD_" withString:@""];
}
