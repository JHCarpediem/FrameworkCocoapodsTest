...
//
//    UIImage * image = [UIImage imageNamed:[NSString stringWithFormat:@"Diagnosis.bundle/image/%@",imageName] inBundle:bundle compatibleWithTraitCollection:nil];
//    return image;
}
...
- (void)readTroubleJson {
    // 获取文件路径
    NSString *path = [[NSBundle bundleForClass:[self class]] pathForResource:@"Diagnosis.bundle/故障码状态" ofType:@"json"];
    // 将文件数据化
...
+ (NSDictionary *)readVehicleEventJson {
    // 获取文件路径
    NSString *path = [[NSBundle bundleForClass:[self class]] pathForResource:@"Diagnosis.bundle/vehicleEventID" ofType:@"json"];
    // 将文件数据化
    NSData *data = [[NSData alloc] initWithContentsOfFile:path];
...
+ (NSDictionary *)readVehicleEventTypeJson {
    // 获取文件路径
    NSString *path = [[NSBundle bundleForClass:[self class]] pathForResource:@"Diagnosis.bundle/vehicleEventType" ofType:@"json"];
    // 将文件数据化
    NSData *data = [[NSData alloc] initWithContentsOfFile:path];
    NSDictionary *dict = [NSJSONSerialization JSONObjectWithData:data options:NSJSONReadingMutableLeaves error:nil];
    return dict?:@{};
