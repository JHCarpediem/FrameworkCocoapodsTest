...
@objc public extension NSString {
    @objc public var TDDLocalized: String {
        ...
        guard let url = framework.url(forResource: "Diagnosis", withExtension: "bundle"), let bundle = Bundle(url: url) else{
           return key
        }
...
@objc public class TDDLocalized: NSObject {
    ...
    /** VCI账号解锁 */
    @objc public static var VCI_unlock: String { "VCI_unlock".TDDLocalized }
    ...
    /** 1.识别到该车配备了SGW(安全网关)模块，部分功能需要解锁该功能才能进行。\n\n2.解锁安全网关需要您的设备连接到网络并登录有解锁权限的账号。 */
    @objc public static var fca_gateway_hint_DeepDiag: String { "fca_gateway_hint_DeepDiag".TDDLocalized }

    /** 1.安全网关当前处于锁定状态，请先进行网关解锁以便执行如特殊功能、主动测试以及向ECU写入存储数据等高阶诊断功能。\n\n2.解锁安全网关需要您的设备连接到网络并登录有解锁权限的账号。\n\n3.雷诺网关解锁包的有效期为一年。并且对于同一车辆识别码（VIN）在 24 小时内的三次连接仅做一次计费（从法国时间当天零点至晚上 23:59）。 */
    @objc public static var gateway_buy_tips_neutral_DeepDiag: String { "gateway_buy_tips_neutral_DeepDiag".TDDLocalized }

    /** 1.安全网关当前处于锁定状态，请先进行网关解锁以便执行如特殊功能、主动测试以及向ECU写入存储数据等高阶诊断功能。\n\n2.解锁安全网关需要您的设备连接到网络并登录有解锁权限的账号。 */
    @objc public static var renault_gateway_hint_new_nissan_DeepDiag: String { "renault_gateway_hint_new_nissan_DeepDiag".TDDLocalized }

    /** 1.安全网关当前处于锁定状态，请先进行网关解锁以便执行如特殊功能、主动测试以及向ECU写入存储数据等高阶诊断功能。\n\n2.解锁安全网关需要您的设备连接到网络并登录有解锁权限的账号。\n\n3.该功能需要进行双重身份验证。您需要在个人中心或有功能提示时按照指示进行身份验证。正常情况下，一旦解锁模块，同一次APP运行过程中将保持90分钟，在此过程中无需重复解锁。 */
    @objc public static var vw_gateway_hint_DeepDiag: String { "vw_gateway_hint_DeepDiag".TDDLocalized }
    ...
    /** VCI账号 */
    @objc public static var VCI_account: String { "VCI_account".TDDLocalized }
