...
#define DF_ID_HIDDEN_DL_ERR                  0x80000021   // 特定界面返回值，刷隐藏数据包下载失败
...
// 软件功能状态，例如软件已过期等
#define DF_ST_FOFT_NORMAL                        0x00000000  // 正常状态
#define DF_ST_FOFT_EXPIR                         0x00000001  // 软件过期
#define DF_ST_FOFT_DISABLE                       0x00000003  // 失能状态


// 标明此系统软件的状态，软件已过期、失能等
#define DF_ST_SYS_NORMAL                        DF_ST_FOFT_NORMAL   // 正常状态
#define DF_ST_SYS_EXPIR                         DF_ST_FOFT_EXPIR    // 软件过期
#define DF_ST_SYS_DISABLE                       DF_ST_FOFT_DISABLE  // 失能状态
...
#define DF_ST_MENU_NORMAL                   DF_ST_FOFT_NORMAL    // 正常状态
#define DF_ST_MENU_EXPIR                    DF_ST_FOFT_EXPIR     // 软件过期
#define DF_ST_MENU_DISABLE                  DF_ST_FOFT_DISABLE   // 失能状态
...
///////////////////////////////////////////////////////////////////////////////////////////
/*-------------------------------------------------------------------------------------------
说    明：用于artiShowMsgBox接口，形参uType，指定UI消息类型，或指定功能UI实现和对应功能实现
          接口不受产品限制，根据不同产品有不同实现，App根据指定的消息类型，绘制指定UI
-------------------------------------------------------------------------------------------*/
enum eMsgBoxType
{
...
enum eSpecialShowType
{
    ...
    SST_FUNC_VW_SFD_AUTH  = 3, // 大众网关解锁登录界面，VW SFD 登录界面

    // DEMO演示使用的登录界面（不会真正进行算法服务请求）
...
{
    DS_HIDDEN_FUN_ZIP_DIR       = 0,     //  指定功能ID的刷隐藏软件小包下载
    DS_HIDDEN_PATH_ID           = 1,     //  刷隐藏路径识别包下载文件路径
};
...
///////////////////////////////////////////////////////////////////////////////////////
/*-----------------------------------------------------------------------------------
说    明：数据流项信息结构体，适用于跟数据流项相关的组件，例如 ArtiLiveData 等类
          ArtiLiveData的接口 SetInformation
------------------------------------------------------------------------------------*/
enum // 数据流项信息的字段类型枚举值
{
    E_LD_INFO_NORMAL_STATUS   = 0,   // 正常状态
    E_LD_INFO_DIAG_REMARK     = 1,   // 诊断备注
    E_LD_INFO_REF_VALUE       = 2,   // 参考值
};

struct stLdInformation  // 数据流项的相关信息
{
    uint32_t uKey;           // 数据流项信息的字段类型，枚举值
                             // 例如 0 表示"正常状态", 
                             //      1 表示"诊断备注", 
                             //      2 表示"参考值"

    std::string strValue;    // 数据流项信息的字段类型对应的值
};
...
struct stHiddenItem
{
    ...
    std::string strIconPath;    // 菜单项对应的图标路径，App根据对应的路径加载显示icon
                                // 绝对路径，App存储菜单图片ICON时，strIconPath应为特殊的预定义好的字符串
                                // 例如 DF_ICON_STR_CHASSIS_ENGINE_CLASS即，"HMM_CHASSIS_ENGINE_CLASS"，表示HMM_CHASSIS_ENGINE_CLASS

    std::vector<stHiddenNode> vctNodes;    // 二级菜单项集合
};

// 功能大类中，eHiddenMenuMask 枚举对应的图标路径字符串值，用于stHiddenItem结构体的strIconPath，指定App图标路径字符串值
#define DF_ICON_STR_CHASSIS_ENGINE_CLASS  "HMM_CHASSIS_ENGINE_CLASS"  // HMM_CHASSIS_ENGINE_CLASS   = (1 << 0),   Chassis & Engine               底盘&动力
#define DF_ICON_STR_DRIVING_STEER_CLASS   "HMM_DRIVING_STEER_CLASS"   // HMM_DRIVING_STEER_CLASS    = (1 << 1),   Driving Assist&Steering Wheel  驾驶辅助 （方向盘）
#define DF_ICON_STR_AC_CLASS              "HMM_AC_CLASS"              // HMM_AC_CLASS               = (1 << 2),   Heater & A/C                   空调&加热
#define DF_ICON_STR_IM_CLASS              "HMM_IM_CLASS"              // HMM_IM_CLASS               = (1 << 3),   Instruments                    仪表&中控&多媒体
#define DF_ICON_STR_LIGHTS_CLASS          "HMM_LIGHTS_CLASS"          // HMM_LIGHTS_CLASS           = (1 << 4),   Lights                         灯光
#define DF_ICON_STR_LOCK_CLASS            "HMM_LOCK_CLASS"            // HMM_LOCK_CLASS             = (1 << 5),   Lock                           锁
#define DF_ICON_STR_MIRRORS_CLASS         "HMM_MIRRORS_CLASS"         // HMM_MIRRORS_CLASS          = (1 << 6),   Mirrors                        后视镜
#define DF_ICON_STR_DOOR_CLASS            "HMM_DOOR_CLASS"            // HMM_DOOR_CLASS             = (1 << 7),   Windows&Door&Sunroof           门&窗
#define DF_ICON_STR_WIPERS_CLASS          "HMM_WIPERS_CLASS"          // HMM_WIPERS_CLASS           = (1 << 8),   Wipers & Washer                雨刮
#define DF_ICON_STR_SEATS_CLASS           "HMM_SEATS_CLASS"           // HMM_SEATS_CLASS            = (1 << 9),   Seats&Belt                     座椅&安全带
#define DF_ICON_STR_WARNING_OTHER_CLASS   "HMM_WARNING_OTHER_CLASS"   // HMM_WARNING_OTHER_CLASS    = (1 << 10),  Warnings & Other               告警及其他
