...
public:
    ...
    // 针对入口类型，用于接口GetObdEntryType的返回值，适用于所有产品
    enum class eObdEntryType :uint32_t
    {
        ...
        // “TDart” 快捷方式进入
        OET_TDARTS                = (1 << 23),  // TDARTS、T-Kunai 路径方式进入,  1 << 23
        ////////////////////////////////////////////////////////////////////////////////////////


        // 当前App不支持此入口功能接口，应用先判断是否等于此值，不相等再判断是否等于哪个掩码
...
public:
    ...
    *
    ...
    /*-------------------------------------------------------------------------------
    功    能： 获取当前OBD车型或者其它车型的入口类型
    
               应用先判断是否等于OET_APP_NOT_SUPPORT此值，不相等再判断是否等于哪个掩码
    ...
    返 回 值： 
               ...
               OET_DIAG_DIAG                    “汽车诊断”路径方式进入
               OET_DIAG_MAINTENANCE             “汽车保养”路径方式进入
               OET_DIAG_IMMO                    “汽车防盗”路径方式进入
               OET_MOTOR_DIAG                   “摩托车诊断”路径方式进入
               OET_MOTOR_MAINTENANCE            “摩托车保养”路径方式进入
               OET_MOTOR_IMMO                   “摩托车防盗”路径方式进入
               OET_ADAS                         “ADAS”路径方式进入
     ...
     *
     *     参数说明：eDlScenariosType eType      定下载类型
     *               eType为DS_HIDDEN_PATH_ID时，是刷隐藏路径识别包下载路径
     *                                           下载刷隐藏数据包失败，返回空路径
     * 
