...
public:
    ...
    {
        HFS_ENABLE  = 0,    // 默认状态，可使用
        HFS_DISABLE = 1,    // 不可用状态，不可点击，置灰状态
    };
...
public:
    ...
    *    功  能：设置标题文本的文本提示
    *
    *    参  数：strTitleTips     对应的标题文本的文本提示
    *                             如果strTitleTips为空串，则不显示标题的文本提示
    *
    *    返回值：DF_FUNCTION_APP_CURRENT_NOT_SUPPORT，当前APP版本还没有此接口
    *            DF_FUNCTION_APP_CURRENT_NOT_SUPPORT值由SO返回
    * 
    ...
    *
    *    说  明：如果没有设置（没有调用此接口），则不显示提示内容
    *            如果strTitleTips为空串，则取消之前的接口设置，即没有标题的文本提示
    ****************************************************************************************/
    ...
    *    功  能：设置开关型功能框的状态
    *
    *    参  数：uIndex      哪个开关型功能框的索引，AddSwitchValue返回的偏移
    *            bStatus     输入框的状态
    *
    *                        HFS_ENABLE  = 0,     默认状态，可使用
    *                        HFS_DISABLE = 1,     不可用状态，不可点击，置灰状态
    ...
    *    功  能：设置选择性功能框的状态
    *
    *    参  数：uIndex      哪个选择性功能框的索引，AddSelectedValue返回的偏移
    *            bStatus     输入框的状态
    *
    *                        HFS_ENABLE  = 0,     默认状态，可使用
    *                        HFS_DISABLE = 1,     不可用状态，不可点击，置灰状态
    ...
    *    功  能：设置进度条选择的状态
    *
    *    参  数：uIndex      哪个进度条的索引，AddSlideValue返回的偏移
    *            bStatus     输入框的状态
    *
    *                        HFS_ENABLE  = 0,     默认状态，可使用
    *                        HFS_DISABLE = 1,     不可用状态，不可点击，置灰状态
    ...
    *    功  能：设置输入框的状态
    *
    *    参  数：uIndex      哪个输入框的索引，AddInputBox返回的偏移
    *            bStatus     输入框的状态
    *
    *                        HFS_ENABLE  = 0,     默认状态，可使用
    *                        HFS_DISABLE = 1,     不可用状态，不可点击，置灰状态
    ...
    *    功  能：设置需要联动处理的功能框
    *            
    *            例如，设置了FHT_COMPOSITE_TYPE复合类型，包含了开关型功能，选择型功能，
    *                  可滑动调节型功能，可输入型功能
    *                  此时，需要选择型功能框，可滑动调节型功能框，可输入型功能框，这3个
    *                  功能框的值，是联动变化的，即用户改变任何一个值，其它功能框的值也会
    *                  随着改变
    * 
    *                  SetBindChange(0, 1, 2);
    *
    *    参  数：uSlide      需要联动变化的滑动调节性功能框的索引，AddSlideValue返回的偏移
    *            uInput      需要联动变化的可输入型功能框的索引，AddInputBox返回的偏移
    *            uSelected   需要联动变化的选择型功能框的索引，AddSelectedValue返回的偏移
    *
    *            如果只需要2个功能框联动变化，则将不需要联动的功能框索引设置为-1
    *
    *    返回值：DF_FUNCTION_APP_CURRENT_NOT_SUPPORT，当前APP版本还没有此接口
    *            DF_FUNCTION_APP_CURRENT_NOT_SUPPORT值由SO返回
    *
    ...
    *************************************************************************************/
    ...
    /***************************************************************************
    *    功  能：添加功能介绍，图片和提示文本，App会根据 strSearchId 去下载刷隐
    *            藏功能介绍文件，然后显示
    *            
    *            AddFuncDescribe接口已废弃
    *
    *    参  数：strSearchId    下载刷隐藏功能介绍文件的检索ID
    *                           /api/v1/user/customize/intro
    * 
    *            strPicturePath 指定图片路径
    ...
    ****************************************************************************************/
    ...
    /****************************************************************************************
    *    功  能：针对可输入型功能，即输入框，可输入的数值，FHT_INPUT_TYPE类型
    *            或者FHT_COMPOSITE_TYPE，设置输入框的范围
    *
    *    参  数：Index     对应AddInputBox返回的偏移
    *            minValue  可输入框的最小值
    *            maxValue  可输入框的最大值
    *
    *    返回值：DF_FUNCTION_APP_CURRENT_NOT_SUPPORT，当前APP版本还没有此接口
    *            DF_FUNCTION_APP_CURRENT_NOT_SUPPORT 值由SO返回
    * 
    *            DF_APP_CURRENT_NOT_SUPPORT_FUNCTION, 当前App有此接口，但未实现对应功能
    * 
    *            如果不存在对应的Index，返回-1
    * 
    *    说  明：如果没有调用 AddInputBox, 调用SetInputBoxLimits，返回-1
    ****************************************************************************************/
