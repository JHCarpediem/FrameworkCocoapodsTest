...
* 文件说明 : 文档类标识
* 功能描述 : ArtiDiag900顶部导航栏接口定义
* 创 建 人 : sujiya 20231108
* 实 现 人 : 
* 审 核 人 : binchaolin
* 文件版本 : V1.00
...
// 在界面中显示顶部导航栏，达到一个顶部菜单栏效果
// 
// 通常情况下，大屏平板（12寸或12寸以上）中使用效果较好
/*
    组件名：CArtiTopTap

    顶部导航栏，也称为“顶部导航栏”，ADAS需求增加
    1、从导航体系的层级来分，“左侧导航栏”属于界面的一级导航，“顶部导航栏”属于二级导航
    2、可任意添加导航栏项Tap，一个Tap中可以是一个MsgBox，也可以是一个Input，List等
    3、顶部导航栏，应用可置灰某项Tap，置灰后Tap将不可点击
    5、如果点击Tap，show将依次返回back，直至“顶部导航栏”的show返回Tap索引
       例如：“顶部导航栏”增加了2个TAP项，一个添加了MsgBox，一个添加了List，此时界
       面正在第一个TAP的MsgBox，当点击第二个TAP时，MsgBox的show应返回back，然后
       CArtiTopTap返回指定TAP索引（这里为1），其它依次类推
    6、Show是非阻塞的
*/
...
public:
    ...
    ********************************************************************************/
    ...
    /********************************************************************************
    *    功  能：设置顶部导航栏项（TAP)的类型，并且绑定对应类型的对象指针
    *
    ...
    ***************************************************************************/
    ...
    /*******************************************************************************
    *    功  能：显示导航栏，是非阻塞的（无阻塞场景）
    * 
    *    参  数：无
    * 
    *    返回值：uint32_t 导航栏选项返回值
    * 
    *            DF_ID_TAP_0    点击了第1个TAP
    *            DF_ID_TAP_1    点击了第2个TAP
    *            DF_ID_TAP_2    点击了第3个TAP
    *            DF_ID_TAP_3    点击了第4个TAP
    *            DF_ID_TAP_4    点击了第5个TAP
    *            DF_ID_TAP_5    点击了第6个TAP
    *            DF_ID_TAP_6    点击了第7个TAP
    *            ...........    ..............
    *
    *            按钮为TAP组件的按钮返回，不在此返回
    *
    *    说 明
    *           举例1
    *           当前有2个顶部标签TAP(TAP0和TAP1)，其中TAP0绑定了CArtiMsgBox，
    *           TAP1绑定了CArtiList，当前界面（代码）处于TAP0的CArtiMsgBox的Show中，
    *           且为阻塞Show，此时可点击部分包括：
    *           1、属于CArtiMsgBox的按钮（右下角，包括回退），
    *           2、属于CArtiTopTap的可点击部分，可能是2个TAP
    *           如果界面用户点击了CArtiMsgBox的按钮，在CArtiMsgBox的Show中将返回对应
    *           的按钮值
    *           如果界面用户点击了CArtiTopTap的TAP1，在CArtiMsgBox的Show中将返回对应
    *           的DF_ID_BACK（回退），然后在CArtiMsgBox的Show中返回DF_ID_TAP_1
    ********************************************************************************/
    uint32_t Show();
