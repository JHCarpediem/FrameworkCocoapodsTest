...
namespace Topdon_AD900_Demo {
	...
	uint8_t CArtiGlobalTest::ShowMenu()
	{
		...
		uiMenu.AddItem("GetDiagEntryTypeEx");			vctMenuID.push_back(18);
		...
		while (1)
		{
			...
			else if (uRetBtn < vctMenuID.size())
			{
				...
				{
					ArtiGlobalTest_GetDiagEntryTypeEx();
				}
	...
	void CArtiGlobalTest::ArtiGlobalTest_GetDiagEntryTypeEx()
	{
		vector<bool> vctbSupportFun = CArtiGlobal::GetDiagEntryTypeEx();
		string		strFun;

		for (uint32_t i=0; i<vctbSupportFun.size(); i++)
		{
			string	strType;

			strType = "Fun ";
			strType += uint32ToString(i);
			strType += ":";
		...
		artiShowMsgBox("DiagEntryType", strFun);
	}
	...
	{
		eDlScenariosType eType = DS_HIDDEN_FUN_ZIP_DIR;
		string SwCode = "";
		string FunId = "01";
